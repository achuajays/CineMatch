...
const WatchedMoviesPage: React.FC = () => {
  ...
  const [filteredMovies, setFilteredMovies] = useState<WatchedMovie[]>([]);
  ...
  useEffect(() => {
    const loadImages = async () => {
      const imagePromises = watchedMovies.map(async (movie) => {
        try {
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          imageMap[watchedMovies[index].id] = getPosterUrl(watchedMovies[index].id);
        }
      });
  ...
  const handleMovieClick = (movie: WatchedMovie) => {
    // Navigate to movie details if available
    navigate(`/watched-movie/${movie.id}`, { state: { movie } });
  };
        ...
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#2c3135] px-4 md:px-10 py-3">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex items-center gap-9">
              ...
              <a className="text-green-400 text-sm font-medium leading-normal" href="#">Watched</a>
            ...
            <div className="flex flex-wrap justify-between items-center gap-3 p-4">
              <div>
                ...
                <p className="text-[#a2abb3] text-sm mt-1">
                  {filteredMovies.length} {filteredMovies.length === 1 ? 'movie' : 'movies'} you've watched
                </p>
              </div>
            </div>
            ...
            {filteredMovies.length === 0 ? (
              <div className="text-center py-16">
                <div className="w-24 h-24 mx-auto mb-6 bg-[#2c3135] rounded-full flex items-center justify-center">
                  <Eye size={32} className="text-[#a2abb3]" />
                </div>
                <h3 className="text-white text-xl font-bold mb-2">
                  {searchQuery ? 'No movies found' : 'No watched movies yet'}
                </h3>
                <p className="text-[#a2abb3] text-base mb-6">
                  {searchQuery 
                    ? 'Try adjusting your search terms' 
                    : 'Start watching movies and mark them as watched to track your viewing history'
                  }
                </p>
                {!searchQuery && (
                  <button 
              ...
              <div className="space-y-4">
                {filteredMovies.map((movie) => (
                  <div 
                    key={movie.id}
                    className="group flex items-center gap-4 bg-[#121516] hover:bg-[#1a1f24] px-4 py-4 rounded-xl transition-all duration-300 cursor-pointer"
                  >
                    {/* Movie Poster */}
                    <div 
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <h3 className="text-white text-base md:text-lg font-bold leading-tight line-clamp-1 group-hover:text-green-400 transition-colors">
                          {movie.name}
                        </h3>
                        <span className="flex-shrink-0 bg-green-600 text-white text-xs font-medium px-2 py-1 rounded-full">
                          {movie.genre}
                        </span>
                      </div>
