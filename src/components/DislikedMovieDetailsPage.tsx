...
const DislikedMovieDetailsPage: React.FC = () => {
  const { movieId } = useParams<{ movieId: string }>();
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get movie from location state
  const movie = location.state?.movie as DislikedMovie;
  ...
  const handleRemoveFromDisliked = () => {
    if (removeFromDisliked(movie.id)) {
      navigate('/disliked');
    }
  };
      ...
      <div className="layout-container flex h-full grow flex-col">
        ...
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#2c3135] px-4 md:px-10 py-3">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex items-center gap-9">
              ...
              <button 
                onClick={() => navigate('/disliked')}
                className="text-red-400 text-sm font-medium leading-normal flex items-center gap-2"
              >
                <ArrowLeft size={16} />
                Disliked Movies
              </button>
        ...
        <div className="px-4 md:px-40 flex flex-1 justify-center py-5">
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
            ...
            <button 
              onClick={() => navigate('/disliked')}
              className="md:hidden flex items-center gap-2 text-white mb-4 hover:text-red-400 transition-colors"
            >
              <ArrowLeft size={20} />
              Back to Disliked Movies
            </button>
                ...
                <div className="flex w-full min-w-72 grow flex-col items-stretch justify-center gap-4 py-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="bg-red-600 text-white text-sm font-medium px-3 py-1 rounded-full whitespace-nowrap">
                        {movie.genre}
                      </span>
            ...
            <div className="px-4 py-6">
              <div className="bg-[#2c3135] rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <span className="text-[#a2abb3]">Disliked on:</span>
                    <span className="text-white ml-2">{formatDate(movie.dislikedAt)}</span>
                  </div>
                </div>
              </div>
            </div>
...
export default DislikedMovieDetailsPage;