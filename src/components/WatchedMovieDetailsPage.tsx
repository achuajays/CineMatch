...
import { ArrowLeft, Search, Bell, Star, Calendar, Clock, Eye, Trash2, ExternalLink } from 'lucide-react';
import { WatchedMovie, removeFromWatched } from '../services/watchedMoviesService';
import { getMovieImage } from '../services/imageService';
import RecommendationButton from './RecommendationButton';

const WatchedMovieDetailsPage: React.FC = () => {
  const { movieId } = useParams<{ movieId: string }>();
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get movie from location state
  const movie = location.state?.movie as WatchedMovie;
    ...
    if (movie) {
      const loadImage = async () => {
        try {
          ...
          setPosterUrl(getPosterUrl(movie.id));
        } finally {
  ...
  if (!movie) {
    return (
      <div className="min-h-screen bg-[#121416] text-white flex items-center justify-center">
  ...
  const handleRemoveFromWatched = () => {
    if (removeFromWatched(movie.id)) {
      navigate('/watched');
    }
  };

  // Generate a movie poster URL based on the movie ID
  ...
  return (
    <div className="relative flex size-full min-h-screen flex-col bg-[#121416] text-white overflow-x-hidden" style={{ fontFamily: '"Space Grotesk", "Noto Sans", sans-serif' }}>
      <div className="layout-container flex h-full grow flex-col">
        ...
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#2c3135] px-4 md:px-10 py-3">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex items-center gap-9">
              ...
              <button 
                onClick={() => navigate('/watched')}
                className="text-green-400 text-sm font-medium leading-normal flex items-center gap-2"
              >
                <ArrowLeft size={16} />
                Watched Movies
              </button>
        ...
        <div className="px-4 md:px-40 flex flex-1 justify-center py-5">
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
            ...
            <button 
              onClick={() => navigate('/watched')}
              className="md:hidden flex items-center gap-2 text-white mb-4 hover:text-green-400 transition-colors"
            >
              <ArrowLeft size={20} />
              Back to Watched Movies
            </button>
                ...
                <div className="flex w-full min-w-72 grow flex-col items-stretch justify-center gap-4 py-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="bg-green-600 text-white text-sm font-medium px-3 py-1 rounded-full whitespace-nowrap">
                        {movie.genre}
                      </span>
            ...
            <div className="px-4 py-6">
              <div className="bg-[#2c3135] rounded-xl p-6">
                <h3 className="text-white text-lg font-bold mb-4">Movie Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-[#a2abb3]">Genre:</span>
                    <span className="text-white ml-2">{movie.genre}</span>
                  </div>
                  <div>
                    <span className="text-[#a2abb3]">Duration:</span>
                    <span className="text-white ml-2">{Math.floor(90 + Math.random() * 60)}m</span>
                  </div>
                  <div>
                    <span className="text-[#a2abb3]">Rating:</span>
                    <span className="text-white ml-2">★ {(8.0 + Math.random() * 2).toFixed(1)}</span>
                  </div>
                  <div>
                    <span className="text-[#a2abb3]">Watched on:</span>
                    <span className="text-white ml-2">{formatDate(movie.watchedAt)}</span>
                  </div>
                </div>
              </div>
            </div>
...
export default WatchedMovieDetailsPage;